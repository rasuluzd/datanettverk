...
def log_message(message_text):
    ...
    Use: Uses datetime for timestamp. Output to console.
    Returns: None. Why?: Logging procedure.
    """
...
class DRTPPacket:
    ...
    def __init__(self, seq_num=0, ack_num=0, flags=0, recv_window=0, data_payload=b''):
        ...
        Arguments: seq_num, ack_num, flags, recv_window (int), data_payload (bytes).
        Use: Masks inputs to 16-bit. Ensures RST flag is 0.
    ...
    def pack(self):
        """
        Description: Packs header and appends data to bytes.
        Arguments: None.
        Use: Uses _header_format_string, self.data.
        Returns: bytes: Complete packet. Why?: For UDP send.
        """
        try:
            ...
            log_message(f"Error: Failed to pack DRTP header - {e}")
            raise 
    ...
    def unpack(raw_packet_bytes):
        """
        Description: Parses raw bytes into DRTPPacket object.
        Arguments: raw_packet_bytes (bytes).
        Use: Uses HEADER_LENGTH, _header_format_string.
        Returns: DRTPPacket or None. Why?: Structured packet data or parse failure.
        """
        ...
        try:
            ...
            # Log minimal error, as verbose logging is reduced.
            # log_message(f"Warning: Error unpacking DRTP header - {e}.") 
            return None 
...
class DRTPServer:
    """
    Description: Implements DRTP server logic.
    """
    def __init__(self, ip_addr, port_num, discard_packet_seq=None):
        """
        Description: Initializes server.
        Arguments: ip_addr (str), port_num (int), discard_packet_seq (int, optional).
        Use: Sets up socket, GBN state, metrics.
        Returns: None. Why?: Constructor.
        """
        ...
        self.output_filename = "received_file.dat" # Simplified filename
    ...
    def start(self):
        ...
        Arguments: None.
        Use: Calls internal methods for connection phases. Handles exceptions.
        Returns: None. Why?: Orchestrates server.
        """
        try:
    ...
    def _establish_connection_server(self):
        """
        Description: Server-side 3-way handshake.
        Arguments: None.
        Use: Manages SYN, SYN-ACK, ACK. Sets server state.
        Returns: bool: True if success. Why?: To proceed to data transfer.
        """
        try:
            ...
            if not syn_pkt or not (syn_pkt.flags & FLAG_SYN):
                return False # Minimal logging for failure here
            ...
            log_message("Server: Timeout in handshake.") # Keep important failure logs
            return False
        except Exception: # Catch other handshake errors minimally
            return False

    def _receive_file_data(self):
        """
        Description: Receives file data using GBN.
        Arguments: None.
        Use: Manages expected_seq_num, discard logic, file I/O.
        Returns: None. Why?: Manages reception state.
        """
        try:
                ...
                while self.connection_active:
                    try:
                        ...
                        if pkt.seq_num == self.expected_seq_num:
                            ...
                            ack_resp = DRTPPacket(ack_num=pkt.seq_num, flags=FLAG_ACK) # Still re-ACK duplicates
                            self.sock.sendto(ack_resp.pack(), self.client_addr)
                        else: 
                        ...
                        if not self.end_time_transfer and self.start_time_transfer > 0:
                            self.end_time_transfer = time.time()
                        break
                    except Exception: # Minimal catch for loop errors
                        pass 
        except IOError:
            log_message(f"Server: File I/O error for '{self.output_filename}'.") # Keep critical error logs
            self.connection_active = False
            if not self.end_time_transfer and self.start_time_transfer > 0:
                self.end_time_transfer = time.time()
        # log_message(f"Server: Data reception ended.") # Can be removed for compactness

    def _send_fin_ack_server(self, client_fin_seq_num):
        """
        Description: Sends FIN-ACK.
        Arguments: client_fin_seq_num (int).
        Use: Constructs and sends FIN-ACK packet.
        ...
        except socket.error: # Minimal error logging
            pass
    ...
    def _calculate_and_log_throughput(self):
        ...
        Arguments: None.
        Use: Uses time and byte count attributes.
        ...
        if self.start_time_transfer > 0 and self.end_time_transfer > self.start_time_transfer and \
           self.total_bytes_received_with_header > 0:
            ...
            # Optional debug info can be removed for compactness
            # log_message(f"(Debug Info: Payload={self.total_bytes_received_payload}B, TotalDRTP={self.total_bytes_received_with_header}B, Time={duration:.2f}s)")
...
class DRTPClient:
    """
    Description: Implements DRTP client logic.
    """
    def __init__(self, s_ip, s_port, file_to_send, win_size):
        """
        Description: Initializes client.
        Arguments: s_ip (str), s_port (int), file_to_send (str), win_size (int).
        Use: Sets up socket, server addr, GBN state.
        Returns: None. Why?: Constructor.
        """
        self.server_ip = s_ip
    ...
    def start(self):
        ...
        Arguments: None.
        Use: Calls internal methods for phases. Handles exceptions.
        Returns: None. Why?: Orchestrates client.
        """
        try:
        ...
        except Exception: # Minimal general error catch
            # log_message(f"Client General Error: {e}") # Can be removed for compactness
            pass
        finally:
    ...
    def _establish_connection_client(self):
        """
        Description: Client-side 3-way handshake.
        Arguments: None.
        Use: Sends SYN, waits SYN-ACK, sends ACK. Adjusts window.
        Returns: bool: True if success. Why?: To proceed to data transfer.
        """
        try:
            ...
            if not syn_ack_pkt or not (syn_ack_pkt.flags & FLAG_SYN and syn_ack_pkt.flags & FLAG_ACK):
                log_message("Connection failed") # Keep essential failure logs
                return False
            ...
            # log_message(f"Client: Server rcv_win={server_rcv_win}. Effective send_win={self.effective_window_size}.") # Can be removed
    ...
    def _send_file_data_gbn(self):
        """
        Description: Sends file data using GBN.
        Arguments: None.
        Use: Manages window, packets, ACKs, timeouts.
        Returns: None. Why?: Manages sending state.
        """
        while not self.eof_reached or self.base_seq_num < self.next_seq_num:
            while self.next_seq_num < self.base_seq_num + self.effective_window_size and \
                ...
                except socket.error: return # Critical send error
                ...
                if self.base_seq_num < self.next_seq_num:
                    ...
                    for seq_to_resend in range(self.base_seq_num, self.next_seq_num):
                        if seq_to_resend in self.sent_packets_buffer:
                            ...
                            except socket.error: return # Critical
            except Exception: # Minimal catch for ACK loop errors
                pass

    def _teardown_connection_client(self):
        """
        Description: Client-side 2-way handshake.
        Arguments: None.
        Use: Sends FIN, waits FIN-ACK, retries.
        Returns: bool: True if success. Why?: Indicates graceful close.
        """
        try:
...
def check_port_arg_main(port_value_str):
    ...
    Use: Converts to int, checks range.
    Returns: int: Valid port. Why?: For argparse. Raises error if invalid.
    """
    try:
...
def main_logic():
    ...
    Arguments: None.
    Use: Uses argparse, DRTP classes.
